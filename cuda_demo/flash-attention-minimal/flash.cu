...
template<>
__global__
void flash_attention_forward_kernel(const float* Q, const float* K, const float* V, const int N, const int d,
...
template<>
__global__
void flash_attention_forward_kernel(const __half* Q, const __half* K, const __half* V, const int N, const int d,
                    const int Tc, const int Tr, const int Bc, const int Br, const float softmax_scale,
                    float* l, float *m, __half* O) {
    int tx = threadIdx.x;
    int bx = blockIdx.x; int by = blockIdx.y;  // batch and head index
    ...
    for (int j = 0; j < Tc; j++) {
        ...
        for (int i = 0; i < Tr; i++)  {
            ...
            float row_l = 0;
            for (int y = 0; y < Bc; y++) {
                S[(Bc * tx) + y] = __float2half(__expf(__half2float(S[(Bc * tx) + y]) - row_m));
            ...
            for (int x = 0; x < d; x++) {
                ...
                float res = (1 / row_l_new) \
                    * ((row_l_prev * __expf(row_m_prev - row_m_new) * __half2float(O[qkv_offset + (tile_size * i) + (tx * d) + x]) ) \
                    + (__expf(row_m - row_m_new) * pv));
...
torch::Tensor flash_attention_forward(torch::Tensor Q, torch::Tensor K, torch::Tensor V) {
    ...
    if (scalar_type == at::kFloat) {
        ...
        flash_attention_forward_kernel<__half><<<grid_dim, block_dim, sram_size>>>(
            reinterpret_cast<__half*>(Q.data<__half>()), reinterpret_cast<__half*>(K.data<__half>()), reinterpret_cast<__half*>(V.data<__half>()),
            N, d, Tc, Tr, Bc, Br, softmax_scale,
            l.data_ptr<float>(), m.data_ptr<float>(), reinterpret_cast<__half*>(O.data<__half>())
        );
    } else {
