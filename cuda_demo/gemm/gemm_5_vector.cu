...
template <>
__global__ void smem_gemm_blocktiling_2d_vector_kernel(const float* A,
...
torch::Tensor smem_gemm_blocktiling_2d_vector(torch::Tensor A,
                                              torch::Tensor B) {
  ...
  for (int i = 0; i < REPEAT_TIME; ++i)
    smem_gemm_blocktiling_2d_vector_kernel<float><<<grid_dim, block_dim>>>(
        A.data_ptr<float>(), B.data_ptr<float>(), C.data_ptr<float>(), M, N, K);
  cudaEventRecord(stop);
  cudaEventSynchronize(stop);
  cudaEventElapsedTime(&milliseconds, start, stop);
  std::cout << "smem_gemm_blocktiling_2d_vector Kernel execution time: "
