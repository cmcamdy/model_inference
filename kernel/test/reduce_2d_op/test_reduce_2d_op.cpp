...
template <typename T, typename RT=int64_t>
std::vector<RT> to_shape(T &shape) {
  std::vector<RT> shape1;
...
TEST(reduceOP2D, PositiveNumbers) {
  using DType = float;
  ...
  std::string file = test_data_path + "/reduce_sum_2d_cuda_test.npz";
  ...
  // auto shapes = vector<int>{256, 2048};
  auto shapes = to_shape<decltype(in.shape), int>(in.shape);
  std::cout << shapes[0] << " " << shapes[1] << std::endl;
  
  // build instance
  auto inst = std::make_unique<kernel::reduce_2d_op::reduceOP2D<DType>>(
      -1, 0, to_shape<decltype(in.shape), int>(in.shape));
