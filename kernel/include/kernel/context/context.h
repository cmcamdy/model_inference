...
enum DeviceType { CPU_DEVICE = 0, CUDA_DEVICE = 1, CUDA_HOST_DEVICE = 2 };
 ...
 * 1. As a wrapper, ContextTensor do not own the `raw_data` memory, users need
 * to manager the memory buffer on their own.
 * 2. `raw_shape` and `raw_dim` must be used together, ContextTensor also do not
 * own the `raw_shape` buffer. Or user can just use `shape`.
 */
struct ContextTensor {
  ...
  /*! \brief The shape of this tensor, if `raw_shape` is not used, this will be
   * used. */
  ...
  inline std::vector<int64_t> get_shape() const {
    return raw_shape ? std::vector<int64_t>(raw_shape, raw_shape + raw_dim)
                     : shape;
  }
...
class CustomOpContext {
 public:
   ...
   */
  virtual TensorPointer AllocateOutput(
  ...
  /*! \brief Copy the input tensor to output based on index, return the output
   * tensor. \param output_idx The output tensor index. \param input_idx The
   * input tensor index. \return The output Tensor.
   */
