...
#define CUDA_CHECK(condition)                                           \
  ...
  do {                                                                  \
    ...
    if (error != cudaSuccess) {                                         \
      std::cout << cudaGetErrorString(error) << " " << __FILE__ << "  " \
                << __LINE__ << std::endl;                               \
      std::exit(-1);                                                    \
    }                                                                   \
  } while (0)
...
#define CUDA_1D_KERNEL_LOOP_EX(i, n, DType)                    \
  for (DType i = blockIdx.x * blockDim.x + threadIdx.x; i < n; \
       i += blockDim.x * gridDim.x)
...
#define CUDA_GET_BLOCKS(N)                                               \
  static_cast<int>(                                                      \
      std::max(std::min(static_cast<size_t>(DIVUP(N, CUDA_NUM_THREADS)), \
                        static_cast<size_t>(CUDA_MAXIMUM_NUM_BLOCKS)),   \
               static_cast<size_t>(1)))

#define CUDA_GET_BLOCKS_EX(N, M)                                       \
  static_cast<int>(                                                    \
      std::max(std::min(static_cast<size_t>(DIVUP(N, M)),              \
                        static_cast<size_t>(CUDA_MAXIMUM_NUM_BLOCKS)), \
               static_cast<size_t>(1)))
