...
VectorAdd1DOp<DType>::VectorAdd1DOp(int vector_len)
    : CustomOpBase(*this), vector_len_(vector_len) {}
...
OP_Status VectorAdd1DOp<DType>::Compute(context::CustomOpContext *context) {
  ...
  CHECK_COND(a_tensor->dims() == 1 && b_tensor->dims() == 1 &&
                 a_tensor->dim_size(0) == b_tensor->dim_size(0),
  ...
  auto output_tensor =
      context->AllocateOutput(0, in_shapes[0], ConvertToDataType<DType>(),
                              context::DeviceType::CUDA_DEVICE);
  DType *output_ptr = static_cast<DType *>(output_tensor->raw_data);
  bool compute_status = functor::VectorAdd1DOpExecute<DType>()(
      a_tensor_ptr, b_tensor_ptr, in_shapes[0][0], output_ptr, stream);
