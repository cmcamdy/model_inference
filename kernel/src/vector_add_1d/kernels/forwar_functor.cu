...
bool VectorAdd1DOpExecute<DType>::operator()(const DType *a_tensor_ptr,
                                             const DType *b_tensor_ptr,
                                             int length, DType *output_ptr,
                                             cudaStream_t &stream) {
  // set block grid
  dim3 blockSize(256);
  dim3 gridSize((length + blockSize.x - 1) / blockSize.x);
  // launch kernel
  cuda_kernels::vector_add_1d_kernel<DType><<<gridSize, blockSize, 0, stream>>>(
      a_tensor_ptr, b_tensor_ptr, output_ptr, length);
  return true;
}
